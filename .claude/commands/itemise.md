...
Locate `CLAUDE.md`, checking the current directory first, then search subdirectories (`find . -maxdepth 4 -name "CLAUDE.md" -type f`) if not found. Use the one that contains `TASK_REGISTRY.md` (confirms it's a Context Guard CLAUDE.md). Look for a line that starts with `ITEMISATION:`.
...
**The principle: one job, one address.** Itemisation is a navigation tool. Its value is proportional to how precisely it can land you on the relevant lines. Every distinct piece of code that does one specific thing, meaning every concern a user might point at and ask "where's the code that does X?" about, gets its own referenceable number.

The test for any numbered block is simple: *if a user asked "where's the code that does X?", could you send them to this number, or would you have to direct them to a sub-part?* If you'd have to direct them inside, the block is too coarse, so sub-number it. The unit-of-work differs by language (functions in JS/PHP, selector blocks in CSS, statements in SQL) but the principle is universal: one job, one address.

**Number these:**

- **Top-level sections** are logical groups of related code. Use a `SECTION:` label:
  ```
...
- **Functions and methods**, each significant function body:
  ...
  // end of 1.1
  ```
- **Significant conditionals** are if/else or switch blocks with meaningful business logic (not trivial single-line guards):
  ```
  // 1.1.1 Return 401 if no auth header present
  if (!authHeader) { return error('no_auth') }
  ```
- **Important loops** are for/while/foreach with non-trivial bodies:
  ```
  // 1.2.1 Process each booking slot
  for (const slot of slots) { ... }
  ```
- **Key configuration objects** are important arrays or objects passed to significant calls:
  ```
...
- A 30-line region containing 3 distinct CSS selector groups (`.header`, `.nav`, `.footer`) → 3 numbers, one per selector, since each affects a different part of the page
...
- Every distinct selector group gets its own number, so `.header { ... }` and `.nav { ... }` are TWO numbers, even if they sit adjacent in the file.
- Pseudo-states (`:hover`, `:focus`, `:before`, `:after`, etc.) get their own numbers, because a hover state is a different visual job than the base.
- `@media` queries get their own numbers, because they describe a separate behavioural context.
...
- The itemisation label and the original comment are separate concerns, and both must appear in the output
...
**Why this step is mandatory:** the Step 5 integrity check verifies that no code was changed, but it does NOT verify granularity. An agent can apply the rules mechanically and end up with one number covering 30 lines of multiple distinct concerns, and Step 5 will still pass. Granularity is a separate failure mode and must be caught here.
...
- The header note is **fixed text** (no date in the header, because regeneration is rare and timestamps live per row).
...
4. For sidecar entries whose numbers **no longer exist** in the source file: delete the row entirely (no tombstones, since git history holds the record).
...
Run this check for each file. The pattern below covers all five comment styles in the language table (`//`, `#`, `/*` for CSS/SCSS/Less, `<!--` for HTML/XML/Vue, and `--` for SQL), so the same command works for every supported language. The command:
